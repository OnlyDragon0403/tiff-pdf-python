...
for filename in files:
    ...
    n_no, n_name , n_status = getNewStep(n_step, total_no , no , "controlling resolution")
    no += 1
    try:
        with Image(filename=filein + filename, resolution=72) as img:
            #img.type = 'grayscale';                 # convert image to grayscale color
            img.type = 'truecolor';                 # convert image to true color
            #img.transform('200x200', '100%')          # cut 200*200 from entire image
        ...
        n_status.text = "resolution convertion success"
    except:
        n_status.text = "resolution convertion failed"
